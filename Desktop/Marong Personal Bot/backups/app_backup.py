...
MT5_SYMBOL_ALIASES = {
    "SA40": ["SA40", "SouthAfrica40", "SouthAfrica40Index"],
    "XPTUSD": ["XPTUSD", "PlatinumUSD", "Platinum"],
}
...
def connect_mt5(account_id: int | None = None, account_password: str | None = None, server: str | None = None) -> bool:
    if mt5 is None:
...
def resolve_mt5_symbol(symbol: str) -> str | None:
    ...
    normalized_aliases = [alias.replace(".", "").replace("_", "").replace("-", "").upper() for alias in aliases]
    for name in names:
        normalized_name = name.replace(".", "").replace("_", "").replace("-", "").upper()
...
def broker_symbol_hints(symbol: str) -> list[str]:
    ...
    keywords = MT5_SYMBOL_ALIASES.get(symbol, [symbol])
...
def load_live_market_snapshot() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for symbol in SYMBOLS:
        ...
        if broker_symbol is None or not mt5.symbol_select(broker_symbol, True):
            ...
            feed_errors.append(f"{symbol}: no matching broker symbol.{hint_text}")
            continue
        symbol_map[symbol] = broker_symbol
        rates = mt5.copy_rates_from_pos(broker_symbol, mt5.TIMEFRAME_M15, 0, 72)
        if rates is None:
...
def render_market_pulse(selected_symbol: str, fallback_market: pd.DataFrame) -> None:
    ...
    with st.container(border=True):
        ...
        st.caption(f"{selected_symbol}  |  last {latest:.5f}  |  15m change {latest - previous:+.5f}  |  SAST / {feed_label}")
